    ...
    const routes = app
        ...
        .get('/processes/:id', async (c) => {
            try {
                ...
                const desc = await pm2Call('describe', isNaN(Number(id)) ? id : Number(id))
        ...
        .get('/processes/:id/logs/stream', async (c) => {
            ...
            const events =
                type === 'out' ? ['log:out'] : type === 'err' ? ['log:err'] : ['log:out', 'log:err']
        ...
        .post(
            ...
            validator('json', (value, c) => {
                ...
                if (!Number.isInteger(instances) || instances < 1) {
                    return c.json({ error: "Body must include an integer 'instances' >= 1" }, 400)
                }
                return { instances }
            }),
            async (c) => {
